...
namespace _3D_Render_Engine_Whit_UI.RenderEngine
{
    class ObjDecoder
    {
        public static _3DObject LoadObj(string path, EngineUi form)
        {
            ...
            List<Vector> normlas = new List<Vector>();
            ...
            foreach (string row in raw_file)
            {
                ...
                }else if (row_splitted[0] == "v")
                {
                    ...
                    vertices.Add(new Vector() { x = x,y = y,z= z});
                ...
                }else if (row_splitted[0].Contains("vn"))
                {
                    ...
                    float y = float.Parse(row_splitted[2].Replace('.', ','));
                    float z = float.Parse(row_splitted[3].Replace('.', ','));
                    normlas.Add(new Vector() { x = x, y = y, z = z });
                    //form.ConsolePrint(row_splitted[0] + "||" + row_splitted[1] + "||" + row_splitted[2] + "||"+row_splitted[3]);
            ...
            {
                Vector[] normals = new Vector[3] { vertices[raw_normals[i] - 1], vertices[raw_normals[i + 1] - 1], vertices[raw_normals[i] - 1 + 2] };
                Face face = new Face() { a = vertices[raw_faces[i]-1], b = vertices[raw_faces[i + 1]-1], c = vertices[raw_faces[i + 2]-1] ,aNormal = normals[0], bNormal = normals[1], cNormal = normals[2], normal = Vector.Ava(normals) };
                faces[i / 3] = face;
                Console.WriteLine(face.a.Print()+"||" + face.b.Print() + "||" + face.c.Print() + "||" + face.aNormal.Print() + "||" + face.bNormal.Print() + "||" + face.cNormal.Print()+"||"+face.normal.Print());
            }
            obj.faces = faces;
